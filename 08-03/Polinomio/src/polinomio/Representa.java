...
public class Representa {
    ...
    public Representa soma(Representa polinomio)
    {
        ...
        {
            if(i <= this.grau) val1 = vetor[i]; else val1 = 0;
            if(i <= polinomio.getGrau()) val2 = polinomio.getElemento(i); else val2 = 0;
     ...
     public Representa multiplica(Representa polinomio)
    {
        ...
        for(int i=0;i<=this.grau;i++)
            for(int j=0;j<=polinomio.getGrau();j++)
                polinomiomult.setElemento(polinomiomult.getElemento(i+j)+ vetor[i]*polinomio.getElemento(j), i+j); 
